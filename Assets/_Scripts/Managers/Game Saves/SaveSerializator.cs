...
public class SaveSerializer : MonoBehaviour
{
    ...
    public static async UniTask<bool> SerializeObjectAsync(JsonSerializer _serializer, GameSave _data)
    {
        await UniTask.RunOnThreadPool(() =>
        {
            StreamWriter file = File.CreateText(GameSaveManager.Instance.saveFilePath);
